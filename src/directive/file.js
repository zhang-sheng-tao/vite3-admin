...
export function isScroll(el, binding) {
  ...
  //                     cur = e.clientY - this._oy; //列表位置应为 鼠标当前位置减去鼠标按下时与列表位置的差值,
  //                     //如:列表初始位置为0,鼠标在 5的位置按,那么差值为 5,此处假如鼠标从5滑动到了4,向上滑,cur = 4-5 =-1  ,假如鼠标从5滑动到了6,向下滑,cur= 6 - 5 = 1
  ...
  //                         cur += this._oh - this._ch; //列表位置加等于 列表高度减容器高度(这是与上面不同,这里是正减,得到了一个正数) ,
  //                         //这里 cur 为负数,加上一个正数,延用上面的假设,此时 cur = -21 + (30-10=20) = -1 ,所以这里算的是溢出数
  //                         //  console.log(cur);
  //                         cur = cur * fl / (fl - cur) - this._oh + this._ch; //然后给溢出数带入弹力,延用上面的假设,
  //                         //这里为cur = -1 * 150 /(150 - -1 = 151)~= -0.99 再减去 30  等于 -30.99  再加上容器高度 -30.99+10=-20.99  ,这也是公式,要死记。。
  //                     }
  //                     setPos(cur); //移动列表
  //                 }
  ...
  //         isInTransition = true;
  //         ctx.timer = setInterval(function () { //回弹算法为  当前位置 减 目标位置 取2个百分点 递减
  //             cur -= (cur - target) * 0.2;
  ...
  //         if (isDown) {
  //             isDown = false;
  //             var t = this, friction = ((vy >> 31) * 2 + 1) * 0.5 //根据力度套用公式计算出惯性大小,公式要记住
